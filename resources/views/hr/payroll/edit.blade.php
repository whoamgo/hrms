        ...
        <div class="card-box">
            <form id="payslip-form" data-ajax-form>
                ...
                <div class="form-group mt-3">
                    ...
                    <a href="{{ route('hr.payroll.index') }}" class="btn btn-danger">Cancel</a>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    $('#payslip-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("hr.payroll.update", $payslip->id) }}',
            type: 'PUT',
            data: formData + '&_token={{ csrf_token() }}',
            success: function(response) {
                if (response.success) {
                    alert(response.message);
                    window.location.href = response.redirect || '{{ route("hr.payroll.index") }}';
                }
            },
            error: function(xhr) {
