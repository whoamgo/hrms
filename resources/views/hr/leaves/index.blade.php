...
<div class="row">
    <div class="col-12">
        <div class="page-title-box flex_btn">
            ...
            <a href="{{ route('hr.leaves.create') }}" class="btn btn-primary">Add Leave</a>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('#filter_employee_id').on('input', function() {
        ...
        $input.data('timeout', setTimeout(function() {
            if (nameCache[term]) {
                ...
                $.ajax({
                    url: '{{ route("hr.leaves.autocomplete.employee-name") }}',
                    data: { term: term },
    ...
    $(document).on('click', '.approve-leave', function() {
        ...
        var url = "{{ route('hr.leaves.approve', ':id') }}";   
    ...
    $('#confirmRejectLeave').on('click', function() {
        if (rejectLeaveId && $('#rejection_reason').val()) {
            ...
            var rejectLeaveUrl = "{{ route('hr.leaves.reject', ':id') }}";   
