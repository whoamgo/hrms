...
class LeaveController extends Controller
{
        ...
        try {
            ...
            foreach ($leaves as $leave) {
                ...
                $data[] = [
                    ...
                    'dates' => $dates,
                    'status' => $leave->status,
                    'actions' => view('admin.leaves.partials.actions', ['leave' => $leave])->render(),
                ];
            }
    ...
    public function create()
    {
        try {
            ...
            return redirect()->route('admin.leaves.index')->with('error', 'Error loading form: ' . $e->getMessage());
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                'redirect' => route('admin.leaves.index')
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function show(Request $request, $id)
{
    try {
        ...
        if ($request->ajax()) {
            return view(
                'admin.leaves.partials.view-modal',
                compact('leave')
            );
        }
     ...
     public function destroy(Request $request, $id)
    {
        ...
        try {
            ...
            //echo Crypt::decryptString($id); die();
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error deleting leave....'.$e->message()
            ], 500);
        }
    }
        ...
        try {
            ...
            if ($leave->employee && $leave->employee->user) {
                NotificationHelper::notify(
                    ...
                    'Leave Approved',
                    "Your {$leave->leave_type} leave from {$leave->from_date->format('d M Y')} to {$leave->to_date->format('d M Y')} has been approved by Admin.",
                    'success',
        ...
        try {
            ...
            if ($leave->employee && $leave->employee->user) {
                NotificationHelper::notify(
                    $leave->employee->user_id,
                    'Leave Rejected',
                    "Your {$leave->leave_type} leave from {$leave->from_date->format('d M Y')} to {$leave->to_date->format('d M Y')} has been rejected by Admin. Reason: {$request->rejection_reason}",
                    'error',
                    route('employee.leaves.create')
                );
            }
